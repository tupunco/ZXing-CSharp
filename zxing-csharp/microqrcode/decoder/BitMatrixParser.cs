...
namespace com.google.zxing.microqrcode.decoder
{
    ...
    sealed class BitMatrixParser
    {
        ...
        internal Version readVersion()
        {
            ...
            var formatInfoVersion = readFormatInformation().Version;
            var dimensionVersion = Version.getProvisionalVersionForDimension(bitMatrix.Dimension);
            if (dimensionVersion != null && dimensionVersion == formatInfoVersion)
            {
                parsedVersion = formatInfoVersion;
                return parsedVersion;
            }
            throw ReaderException.Instance;
        }
