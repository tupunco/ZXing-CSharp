...
namespace com.google.zxing.microqrcode.decoder
{
    ...
    abstract class DataMask
    {
        ...
        private static readonly DataMask[] DATA_MASKS = new DataMask[] { new DataMask00(), new DataMask01(), new DataMask10(), new DataMask11() };
        ...
        private class DataMask00 : DataMask
        {
        ...
        private class DataMask01 : DataMask
        {
            internal override bool isMasked(int i, int j)
            {
                return (((SupportClass.URShift(i, 1)) + (j / 3)) & 0x1) == 0;
            }
        }

        /// <summary> 10: mask bits for which xy mod 2 + xy mod 3 == 0</summary>
        private class DataMask10 : DataMask
        {
            internal override bool isMasked(int i, int j)
            {
                int temp = i * j;
                //INFO Not the same with the QR Code
                return (((temp & 0x1) + (temp % 3)) & 0x1) == 0;
            }
        }
        /// <summary> 11: mask bits for which ((x+y)mod 2 + xy mod 3) mod 2 == 0</summary>
        private class DataMask11 : DataMask
        {
            internal override bool isMasked(int i, int j)
            {
